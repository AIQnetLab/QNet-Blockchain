...
class ProductionCrypto {
    ...
    static async generateMnemonic() {
        try {
            const bip39 = await loadProductionBIP39();
            if (bip39) {
                return bip39.generateMnemonic(128); // 12 words using real 2048 wordlist
            }
...
async function createWallet(password, mnemonic) {
    try {
        ...
        if (!await ProductionCrypto.validateMnemonic(seedPhrase)) {
...
async function importWallet(password, mnemonic) {
    try {
        ...
        if (!await ProductionCrypto.validateMnemonic(mnemonic)) {
            return { success: false, error: 'Invalid mnemonic phrase' };
        }
