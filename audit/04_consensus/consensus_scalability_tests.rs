...
fn test_qnet_tps_capacity() {
    ...
    for (validator_count, expected_tps, phase) in test_scenarios {
        ...
        println!("    • Expected TPS: {:,}", expected_tps);
        println!("    • Measured TPS: {:,} {}", 
            measured_tps, 
            if measured_tps >= expected_tps * 95 / 100 { "✅" } else { "⚠️" }
        );
        println!("    • Effective TPS: {:,}", effective_tps);
    ...
    println!("    • Peak Measured TPS: {} ({})", 
        format!("{:,}", max_achieved_tps).green().bold(),
        peak_phase.yellow()
    );
    println!("    • Theoretical Maximum: {}", format!("{:,}", theoretical_tps).cyan());
    println!("    • QNet Target (400K): {}", format!("{:,}", 400_000).yellow());
