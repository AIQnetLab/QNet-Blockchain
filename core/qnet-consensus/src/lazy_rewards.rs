...
impl PhaseAwareRewardManager {
    ...
    ///
    /// Wall-clock variant — DISPLAY/STATS ONLY. Consensus must use
    /// `pool1_base_emission_at_height`, which derives the halving cycle from chain height and is
    /// therefore identical on every node.
    fn calculate_pool1_base_emission(&self) -> u64 {
        pool1_base_emission_for_cycles(self.calculate_years_since_genesis() / 4)
    }
    ...
    pub fn register_node(&mut self, node_id: String, node_type: NodeType, wallet_address: String) -> Result<(), ConsensusError> {
        ...
        println!("✅ Node registered for rewards: {} owned by wallet: {}...", 
                 node_id, qnet_state::char_prefix(&wallet_address, 8));
                ...
                if owner_wallet != claimant_wallet {
                    return RewardClaimResult {
                        ...
                        message: format!("SECURITY VIOLATION: Node {} belongs to wallet {}..., not {}...", 
                                       node_id, 
                                       qnet_state::char_prefix(&owner_wallet, 8),
                                       qnet_state::char_prefix(&claimant_wallet, 8)),
                ...
                self.pending_rewards
                    ...
                    .and_modify(|existing| {
                        ...
                        println!("[INFO][REWARDS] accumulated node={} new={} total={}", 
                                qnet_state::char_prefix(&summary.node_id, 16),
