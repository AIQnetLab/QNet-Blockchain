...
mod tests {
    ...
    fn test_microblock_creation() {
        ...
        assert_eq!(mb.producer, "producer1");
        assert_eq!(mb.transactions.len(), 0);
    }
    ...
    fn test_macroblock_creation() {
        let consensus_data = ConsensusData {
            commits: HashMap::new(),
            reveals: HashMap::new(),
            next_leader: "leader1".to_string(),
        };
        
        let mb = MacroBlock::new(
            1,
            1234567890,
            [0u8; 32],
            vec![[1u8; 32], [2u8; 32]],
        ...
        assert_eq!(mb.height, 1);
        assert_eq!(mb.micro_blocks.len(), 2);
    }
    
    #[test]
    fn test_light_header_conversion() {
        let mb = MicroBlock::new(
            1,
            1234567890,
            [0u8; 32],
        ...
        let header = mb.to_light_header();
        assert_eq!(header.height, 1);
        assert_eq!(header.tx_count, 0);
        assert_eq!(header.producer, "producer1");
    }
} 