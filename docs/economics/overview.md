...
Mempool dedup keys a Heartbeat on `(node_id, epoch * 10 + subwindow, 7)`, where `subwindow = (anchor_height % 14_400) / 1_440`, so one heartbeat per node per subwindow reaches a block; apply is idempotent on the bitmask either way. An included heartbeat's mark lives 1,620 blocks (its subwindow plus the 90-block admission lag and 90 blocks of slack), and the RPC and gossip doors refuse a heartbeat whose subwindow bit the account already carries.
...
The shard is a pure function of the node id: `light_shard_of(node_id) = u64::from_le_bytes(blake3(node_id)[..8]) % 5`. It is roster-size-independent, so a node's owning genesis never changes as the registry grows, and three genesis nodes own each shard: genesis `00N` is the primary of shard `N-1` and the next two genesis nodes, mod 5, back it up. The node that takes a device's reply sends the attestation to the shard's other owners directly, and each owner commits its own bitmap row: the primary 150 blocks before the epoch ends, the backups at 100 and 50. The primary stands down only once its own row has landed, a backup once any row for the shard has. The reward path bit-ORs the rows, so a backup's cover adds to the primary's and never replaces it.
...
A light node's roster membership for epoch `E` is frozen at `light_roster_cutoff(E) = E * 14_400 + 14_250` — the moment the commit window opens, 150 blocks before the epoch ends. A node whose registration is stamped below that height is in the epoch's roster, including in its own registration epoch and in epoch 0; a node registered in the closing 150 blocks joins from the next epoch. The bitmap builder and every reward reader call the same function, so creator and reader index the same roster.

Pings are scheduled on the block clock, one epoch of 240 slots of 60 blocks: `slot = (height % 14_400) / 60`. A node's slot is re-randomized every epoch as `hash(node_id, epoch) % 232` (from epoch 95; `% 240` before), so it is not predictable across epochs, and it is woken in its primary slot plus the two following slots. The draw leaves out the last slots, so on the regular one-slot tick a node's primary and grace pushes, and the 180 seconds a challenge is answerable for at the very least, end before the commit window opens. A challenge issued earlier stays answerable until that window opens, so a reply minutes late still counts for its epoch; a catch-up push after a stalled tick can still be answered after the bitmap is built, and that reply misses the epoch. Each ping tick reads back every slot passed since the previous tick within the current epoch, up to 15 slots; a larger gap, or a node a whole epoch behind the chain, reads only the grace slots. Anchoring the schedule to height rather than the wall clock puts exactly one primary slot inside every reward epoch.

The wake roster is drawn from the light nodes an owner holds for the shards it covers — every light node the chain has registered, plus any announced over gossip or to its own RPC — and a backup covers a shard only while every owner ranked above it has been silent for `OWNER_SILENT_SECS = 600` s. A node that has already attested in the current epoch is skipped. A node that has attested in one of the last 3 epochs, or that registered within that grace span, is woken by its shard owner; a node dormant longer than that returns by submitting a pull self-attestation, which both records eligibility for the current epoch and restores it to the wake roster. When the epoch just ended has no committed bitmap for a shard an owner covers, the owner also wakes the dormant nodes of every shard it covers, since that silence was the owner's; it keeps doing so while the run of such epochs is at most `RECOVERY_EPOCHS = 3` and stops the sweep once it reaches four. See [mobile wallet](../applications/mobile-wallet.md).

The bitmap transaction is system-typed: `gas_limit` 0 and `gas_price` `u64::MAX`, so it is free and ordered into the block's system lane. Its nonce is `epoch * 10 + shard + 1` rather than an account sequence, so one owner's bitmaps for different shards in the same epoch carry distinct nonces, and nonce ordering is skipped for the type. Uniqueness is per shard and owner: the commitment identity is `{genesis_id}:{signer}` — the shard's genesis id and the signing genesis id (`Transaction::light_bitmap_commitment_id`) — and state refuses a bitmap for that identity at or below an epoch it has already committed, so each owner lands at most one row per shard per epoch. Each owner's row is stored under its own key, where the lowest inclusion height wins.
...
The window recompute that builds and checks each emission window's checkpoint persists the shard set it derives, so a node can serve an epoch's claims whether or not it produced the emission block. A shard set that is missing, or that does not recombine to the certified root, is rebuilt from the retained indices for the newest 128 epochs holding a root, at boot, once per ping slot and in the hourly maintenance pass; the boot and hourly passes also re-derive the previous epoch's super-eligibility index when it is missing here while supers are registered. An epoch the local rebuild cannot restore is fetched shard by shard from a genesis node through `GET /api/v1/rewards/epoch/{epoch}/leafset?shard=N` (oldest first, two epochs per pass at boot and hourly, a 60 s budget per peer) and stored only when the assembled set hashes to this node's own certified `reward_root`, so a peer can affect whether an epoch is servable, never the amount.
...
A wallet POSTs to `/api/v1/rewards/claim` with `node_id`, `wallet_address`, and a mandatory ML-DSA-65 signature over `q{QNET_CHAIN_ID}|claim_rewards:{node_id}:{wallet_address}`; the chain tag `q{QNET_CHAIN_ID}|` (`QNET_CHAIN_ID = 1337`) prefixes both messages a claim signs, so an authorization made on one chain does not verify on another. The request is rejected outright if that signature is missing, and `wallet_address` must pass full EON-address validation before any state is read. The claimant wallet must equal the on-chain registered wallet for the `node_id`; an unregistered node cannot claim. A per-node in-progress lock rejects concurrent claims for the same node.

Without `claims_data`/`claims_signature`, the node quotes a batch. It walks the epoch grid (multiples of 160) strictly above the wallet's `last_claimed_epoch` in ascending order, up to the newest root it holds, resolves each epoch's root from its stored row or, when the row is missing, from the certifying macroblock it holds, generates each merkle proof from the locally stored shard, and stops at the first epoch it cannot serve, reporting `stopped_at_epoch` and `stopped_reason`. Bounds on the quote:
...
```
q{QNET_CHAIN_ID}|qnet_claim_v1:{wallet}:{claim_timestamp}:{hex(SHA3-256(claims_data))}
```
...
| Light nodes | 75% | in the epoch's roster (registration stamped below `epoch_start + 14_250`) and bit set in a `LightNodeEligibilityBitmap` row committed for its shard in that epoch, which one attestation in the epoch achieves |
